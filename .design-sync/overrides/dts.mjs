...
function projectFor(pkgDir, typesRoot) {
  ...
  // FORK (design-sync override): TS-source-only packages declare no `types`
  // (main/exports point at src/*.ts, no committed dist). findTypesRoot already
  // resolved `typesRoot` to the emitted dist/ d.ts tree — honor it for the entry
  // instead of falling back to a non-existent `<pkgDir>/index.d.ts`, which leaves
  // getSourceFile(entry) undefined → 0 exported symbols → 0 components.
  const declared =
    (pubEntry && existsSync(join(pkgDir, pubEntry)) ? join(pkgDir, pubEntry) : null) ||
    (pj.types && existsSync(join(pkgDir, pj.types)) ? join(pkgDir, pj.types) : null) ||
    (pj.typings && existsSync(join(pkgDir, pj.typings)) ? join(pkgDir, pj.typings) : null);
  const entry =
    declared ||
    (typesRoot && existsSync(join(typesRoot, "index.d.ts"))
...
function typeText(t, at) {
  ...
  if (t.isUnion()) {
    ...
    if (uniq.length > 24)
      uniq = [...uniq.slice(0, 16), `(string & {}) /* +${uniq.length - 16} more */`];
      ...
      if (Node.isSourceFile(d)) {
        const members = [...d.getExportedDeclarations().entries()]
          .filter(
            ([n, ds]) =>
              /^[A-Z][a-z]/.test(n) &&
              ds.some((x) => !Node.isInterfaceDeclaration(x) && !Node.isTypeAliasDeclaration(x)),
          )
          .map(([n]) => n);
...
export function smartDefaultProps(name, pb) {
  ...
  for (const m of body.matchAll(/^ {2}([a-zA-Z_$][\w$]*)(\??)\s*:\s*(.+);$/gm)) {
    ...
    // Visibility toggles — an overlay/dialog with open=false renders nothing.
      ...
      /^(open|isOpen|visible|show|defaultOpen|expanded|checked|active|selected)$/.test(prop) &&
      t === "boolean"
    )
      props[prop] = true;
    ...
    // text (string[] → strings; otherwise {id,label,value}).
      ...
      const elIsString = /^(?:readonly\s+)?string\[\]|^ReadonlyArray<string>|^Array<string>/.test(
        elT,
      );
